...
export default class SynonymController {
  ...
  createSynonym = (req: Request, res: Response, next: NextFunction) => {
    try {
      const response: Set<string> = SynonymRepository.getInstance().createSynonym(
        req.body.newWord,
        req.body.existingWord
      );
  ...
  updateSynonym = (req: Request, res: Response, next: NextFunction) => {
    try {
      const response: Set<string> = SynonymRepository.getInstance().updateSynonym(
        req.body.wordToBeUpdated,
        req.body.synonymWord
      );
